...
{   
    ...
    vector<queue<Vertex>> vector_queue(max_distance + 1);
    ...
    {   
        ...
        while (!current.empty())
        {
            Vertex current_front = current.front();
            current.pop();

            if (current_front == end) return distances[end];

            for (Edge& edge : graph.getNeighbors(current_front))
            {
